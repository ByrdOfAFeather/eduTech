...
class AttnGruDecoder(nn.Module):
	...
	def greedy_search(self, hidden_state, encoder_attention, BERT_ENCODER=None):
		...
		while True:
			...
			current_word = \
				BERT_MODEL(torch.tensor(preds[no_outputted, :].unsqueeze(0).type(torch.LongTensor), device='cuda'))[0][
					0]
			...
			for batch_idx in range(0, batch_size):
				for word_idx in range(0, no_words):
					...
					if rand.randint(0, 1) == 1 or word_idx == 0:
						...
						current_word = BERT_MODEL(torch.tensor(
							torch.argmax(preds[batch_idx, word_idx - 1, :]).unsqueeze(0).unsqueeze(0).type(
								torch.LongTensor), device='cuda'))[0][0]
					if word_idx == 0:
						hidden_state = self.gru_module(current_word, hidden_state)
					else:
						hidden_state = self.gru_module(current_word, hidden_state)
